-- ============================================================
-- Ausgabenverwaltung – Schema Version 4
--
-- Massgebliches Schema fuer NEUE Datenbanken. Bestehende
-- Datenbanken der Version 3 werden nicht mit dieser Datei
-- angelegt, sondern mit docs/migration_v3_to_v4.sql
-- hochgezogen. Beide Wege muessen zum selben Ergebnis fuehren
-- (siehe DatabaseMigratorTests).
--
-- Unterschied zu Version 3: Expense.IsIncome und
-- RecurringExpense.IsIncome.
--
...
CREATE TABLE RecurringExpense (
    ...
    -- Neu in Version 4: 0/1, ob die erzeugten Buchungen Einnahmen
    -- statt Ausgaben sind (siehe Expense.IsIncome unten). Wird beim
    -- Erzeugen einer Buchung kopiert wie AmountCents (Regel 6) -
    -- nachtraeglich am erzeugten Vorkommen zu aendern wirkt sich
    -- also nicht auf die Vorlage aus und umgekehrt. Steht hinter
    -- ModifiedUtc statt an der thematisch passenderen Stelle weiter
    -- oben: ALTER TABLE ADD COLUMN haengt neue Spalten immer hinten
    -- an (siehe DatabaseMigratorTests), und alle Spalten-Definitionen
    -- muessen in SQLite ohnehin vor den Tabellen-Constraints (FOREIGN
    -- KEY, CHECK) stehen.
